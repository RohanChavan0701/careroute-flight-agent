...
class FlightAwareProvider:
    ...
    def _map_to_flight_raw(self, data: Dict[str, Any]) -> FlightRaw:
        ...
        sched_dep = self._parse_time(data.get("scheduled_out") or data.get("scheduled_off"))
        est_dep = self._parse_time(data.get("estimated_out") or data.get("estimated_off"))
        sched_arr = self._parse_time(data.get("scheduled_in") or data.get("scheduled_on"))
        est_arr = self._parse_time(data.get("estimated_in") or data.get("estimated_on"))
    ...
    def _parse_time(self, time_str: str | None) -> str | None:
        """Parse ISO time to YYYY-MM-DDTHH:MM format."""
        if not time_str:
            return None
        try:
            # Remove Z and parse
